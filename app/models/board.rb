...
#  replicated_model
  ...
  add_permissions('view') {|user| @sub_id && (self.public || user.can_view?(self)) && @sub_global.allows?(user, 'model') }
  add_permissions('view', 'edit') {|user| @sub_id && (self.public || user.can_view?(self)) && @sub_global.allows?(user, 'edit_boards') }
...
#      BoardDownstreamButtonSet.schedule_once(:update_for, board.global_id)
  ...
  def images_and_sounds_for(user)
    ...
    include_other_sources = user && user.supporter_role?
