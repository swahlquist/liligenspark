...
#    Octopus.using(:master) do
      non_user_params[:user] = User.using(:master).find_by_global_id(non_user_params[:user_id])
      non_user_params[:author] = User.using(:master).find_by_global_id(non_user_params[:author_id])
      non_user_params[:device] = Device.using(:master).find_by_global_id(non_user_params[:device_id])
#    end
    raise "user required" if !non_user_params[:user]
    raise "author required" if !non_user_params[:author]
    raise "device required" if !non_user_params[:device]
