...
var levels = [
  ...
  [
    ...
    {intro: 'literacy', literacy: true, difficulty: 0, always_visual: true, distractors: true, min_attempts: 4}, // find the word for this picture
...
var functional = { // no picture?
  ...
  food: {prompt: "eat for dinner"},
  sleep: {prompt: "use for sleeping"},
  // more specifics (use to cut, sleep on, wear, use to sweep, can fly, you kick, use to paint)
};
var groups = {
  pet: {name: "animals", category: "animal"},
  fruit: {name: "fruits", category: "fruit"},
  body: {name: "body parts", category: "body part"},
  space: {name: "space", category: "space object"},
  shape: {name: "shapes", category: "shape"},
  color: {name: "colors", category: "color"},
  art: {name: "art", category: "art tool"},
  feeling: {name: "feelings", category: "feeling"},
  vehicle: {name: "vehicles", category: "vehicle"},
};
...
obf.register("eval", function(key) {
  ...
  if(assessment.step == 0) {
    ...
    if(step.intro == 'intro') {
      ...
      board.background_text = "Next the evaluation will show simple photographs. Encourage the user to make observations or discuss the picture using the buttons provided";
    }
    ...
    if(step.difficulty != null) {
      if(step.difficulty < 0) {
        ...
        step_cols = 3;
        if(step.find) {
          if(assessment.skill === -1) {
            step_rows = 1;
            step_cols = 2;
          } else if(assessment.skill === 2) {
            step_rows = 1;
            step_cols = 4;
          }
        } else {
          if(assessment.skill === -1 || assessment.skill === 0) { // below-level
            step_rows = 1;
            step_cols = 2;
    ...
    if(step.find) {
      ...
      if(step.find == 'core') {
        ...
        assessment.ref = assessment.ref || {};
        if(!assessment.ref.functional) {
          var categories = [];
          var all_words = [];
          ...
          assessment.ref.all_functional_words = all_words;
          categories = shuffle(categories);
          assessment.ref.functional = categories;
        }
        assessment.ref.functional_index = assessment.ref.functional_index || 0;
        var cat = assessment.ref.functional[assessment.ref.functional_index];
        assessment.ref.functional_index++;
        if(assessment.ref.functional_index >= assessment.ref.functional.length) { assessment.ref.functional_index = 0; }
        ...
        distractor_words = shuffle(assessment.ref.all_functional_words.filter(function(w) { return w.category != cat.category && !(cat.exclude || {})[w.category]; }));
      } else if(step.find == 'functional_association') {
        assessment.ref = assessment.ref || {};
        if(!assessment.ref.functional_associations) {
          var categories = [];
          var all_words = [];
          ...
          assessment.ref.all_functional_association_words = all_words;
          categories = shuffle(categories);
          assessment.ref.functional_associations = categories;
        }
        assessment.ref.functional_association_index = assessment.ref.functional_association_index || 0;
        var cat = assessment.ref.functional_associations[assessment.ref.functional_association_index];
        assessment.ref.functional_association_index++;
        if(assessment.ref.functional_association_index >= assessment.ref.functional_associations.length) { assessment.ref.functional_association_index = 0; }
        if(!cat.prompt) { debugger };
        prompt_text = "What do you do with " + cat.prompt + "?";
        filtered = [cat.action];
        distractor_words = assessment.ref.all_functional_association_words.filter(function(w) { return w != cat.action; });
        board.background_image_url = cat.word.urls['photos'] || cat.word.urls['default'];
      } else if(step.find == 'category') {
        assessment.ref = assessment.ref || {};
        if(!assessment.ref.groups) {
          var categories = [];
          var all_words = [];
          var category_words = [];
          ...
          assessment.ref.all_group_words = shuffle(all_words);
          categories = shuffle(categories);
          assessment.ref.groups = shuffle(categories);
          assessment.ref.category_words = category_words;
        }
        assessment.ref.groups_index = assessment.ref.groups_index || 0;
        var word = assessment.ref.all_group_words[assessment.ref.groups_index];
        var cat = words.find(function(w) { return w.group == word.category});
        filtered = [cat];
        distractor_words = assessment.ref.category_words.filter(function(w) { return w != cat; });
        assessment.ref.groups_index++;
        if(assessment.ref.groups_index >= assessment.ref.all_group_words.length) { assessment.ref.groups_index = 0; }
        ...
        assessment.ref.category_group_index = assessment.ref.category_group_index || 0;
        var category = assessment.ref.groups[assessment.ref.category_group_index];
        assessment.ref.category_group_index++;
        if(assessment.ref.category_group_index >= assessment.ref.groups.length) { assessment.ref.category_group_index = 0; }
        var word = category.words[Math.floor(Math.random() * category.words.length)];
        filtered = [word];
        distractor_words = assessment.ref.all_group_words.concat(category.words);
    ...
    var sample = null;
    if(sample_rand < 0.28 && assessment.cluster_samples && assessment.cluster_samples.length > 0) {
      var idx = Math.floor(Math.random() * assessment.cluster_samples.length);
      sample = assessment.cluster_samples[idx];
    ...
    if(step_rows * step_cols > 8) {
      ...
      if(assessment.n_cnt < edge_cutoff) {
        loc[0] = 0;
      } else if(((rows - 1) * spacing) + offset + skip_rows == step_rows - 1 && assessment.s_cnt < edge_cutoff) {
        loc[0] = rows - 1;
...
//      sound: {}
    ...
    res.handler = function(button, obj) {
      ...
      if(app_state.get('speak_mode')) {
        ...
        if((button.id != 'button_correct' && has_correct_button) || time_to_select > (typical_time_to_select * 5)) {
          ...
          runLater(function() {
            ...
            clusterize(assessment.gravities || []);
            clusterize(assessment.antigravities || []);
            var samples = [];
            ...
            assessment.cluster_samples = samples;
          }, 500);
        }
        var next_step = false;
